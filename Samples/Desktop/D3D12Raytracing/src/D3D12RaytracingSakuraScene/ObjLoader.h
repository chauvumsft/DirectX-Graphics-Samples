#pragma once
#include "RaytracingHlslCompat.h"

class ObjLoader
{
public:
	void Load(wchar_t const* fileName);

	void GetObjectVerticesAndIndices(
		std::string const& name,
		float scale,
		std::vector<Vertex>* vertices,
		std::vector<Index>* indices);

private:
	class Object
	{
	public:
		Object(std::string const& name)
			: m_name(name)
		{
		}

		std::string const& GetName() const { return m_name; }

		void AddVertex(XMFLOAT3 const& vertex)
		{
			m_vertices.push_back(vertex);
		}

		void AddNormal(XMFLOAT3 const& normal)
		{
			m_normals.push_back(normal);
		}

		void AddTexCoord(const XMFLOAT2& texCoord)
		{
			m_texCoords.push_back(texCoord);
		}

		struct ThreeIndices
		{
			unsigned int Values[3];
		};

		struct Face
		{
			ThreeIndices VertexIndices;

			bool UseNormals;
			bool UseTexCoords;
			ThreeIndices NormalIndices;
			ThreeIndices TexCoordIndices;
		};

		// Face with only vertex indices
		void AddFace(ThreeIndices const& vertexIndices)
		{
			Face face;
			face.UseNormals = false;
			face.UseTexCoords = false;
			face.VertexIndices = vertexIndices;
			m_faces.push_back(face);
		}

		// Face with vertex and normal indices
		void AddFace(ThreeIndices const& vertexIndices, ThreeIndices const& normalIndices)
		{
			Face face;
			face.UseNormals = true;
			face.UseTexCoords = false;
			face.VertexIndices = vertexIndices;
			face.NormalIndices = normalIndices;
			m_faces.push_back(face);
		}

		// Face with vertex, normal, and texture coordinate indices
		void AddFace(ThreeIndices const& vertexIndices, ThreeIndices const& normalIndices, ThreeIndices const& texCoordIndices)
		{
			Face face;
			face.UseNormals = true;
			face.UseTexCoords = true;
			face.VertexIndices = vertexIndices;
			face.NormalIndices = normalIndices;
			face.TexCoordIndices = texCoordIndices;
			m_faces.push_back(face);
		}

		std::vector<XMFLOAT3> m_vertices;
		std::vector<XMFLOAT3> m_normals;
		std::vector<XMFLOAT2> m_texCoords;
		std::vector<Face> m_faces;

	private:
		std::string m_name;
	};

	std::vector<Object> m_objects;
	Object* GetObject(std::string const& name);
	Object* GetOrCreateObject(std::string const& name);
};